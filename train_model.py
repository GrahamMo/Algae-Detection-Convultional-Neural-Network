...
def main():
    ...
    algae_filenames = glob("photos/algae_blooms/algaeblooms*")
    lake_filenames = glob("photos/lakes/lakes*")
    algae_images = load_images(algae_filenames)
    lake_images = load_images(lake_filenames)

    algae_labels = np.zeros(len(algae_images))
    lake_labels = np.ones(len(lake_images))

    images = np.concatenate([algae_images, lake_images], axis=0)
    labels = np.concatenate([algae_labels, lake_labels], axis=0)
